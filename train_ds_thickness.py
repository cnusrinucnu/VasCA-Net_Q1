"""
VasCA-Net with Deep Supervision + Thickness-Aware Loss.
...
def build_loss(name: str):
    ...
    if name == "thickness_bce":
        return ThicknessAwareBCELoss(
            alpha=0.25
        )
...
def evaluate(
    ...
    for imgs, masks in loader:
        ...
        # Only final output is evaluated.
        logits = model(
            imgs,
            return_aux=False,
        )
    ...
    for epoch in range(
        ...
        if val_metrics["F1"] > best_f1:
            ...
            ckpt_path = os.path.join(
                cfg["train"]["checkpoint_dir"],
                "vasca_net_ds_thickness_best.pth",
            )
