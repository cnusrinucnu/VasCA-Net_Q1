...
generic assumptions:
  ...
  - CHASE:  images/test_XX_test.tif AND images/training_XX_test.tif
            <-> masks/test_XX_manual1.tif / masks/training_XX_manual1.tif
            (matched by shared prefix, not identical stem)
...
def prep_chase(raw_root: str, out_root: str, n_train: int = 20, seed: int = 42):
    """
    This mirror's CHASE_DB1 files come with a naming quirk: filenames use
    TWO different prefixes -- 'test_XX_test.tif' (8 files) and
    'training_XX_test.tif' (20 files) -- both suffixed '_test.tif' on the
    image side, matched by 'training_XX_manual1.tif' / 'test_XX_manual1.tif'
    on the mask side. We use this naming directly, since it matches the
    paper's reported CHASE_DB1 split (20 train, 8 test) exactly.
    """
    raw_chase = os.path.join(raw_root, "CHASE")
    out_chase = os.path.join(out_root, "CHASEDB1")

    images = sorted(glob.glob(os.path.join(raw_chase, "images", "*_test.tif")))
    train_pairs, test_pairs = [], []
    for img_p in images:
        m = re.match(r"(.+)_test\.tif$", os.path.basename(img_p))
        if not m:
        ...
        if prefix.startswith("test_"):
            test_pairs.append((img_p, mask_p, prefix))
        elif prefix.startswith("training_"):
            train_pairs.append((img_p, mask_p, prefix))
        else:
            print(f"  [WARN] unrecognized prefix '{prefix}', skipping")

    if len(train_pairs) == 0 or len(test_pairs) == 0:
        import random
        print("  [WARN] naming-based split didn't yield both a train and test "
              "set as expected -- falling back to a random split instead.")
        all_pairs = [(i, m, p) for (i, m, p) in (train_pairs + test_pairs)]
        rng = random.Random(seed)
        rng.shuffle(all_pairs)
        train_pairs, test_pairs = all_pairs[:n_train], all_pairs[n_train:]

    for split, subset in [("train", train_pairs), ("test", test_pairs)]:
        for img_p, mask_p, prefix in subset:
            _copy_pair(img_p, mask_p,
